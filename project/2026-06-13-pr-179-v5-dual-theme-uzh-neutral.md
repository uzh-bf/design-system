...
- [x] S7 (post-review, agent-browser pass) Dark-mode status surfaces. agent-browser review of 18 components × neutral/uzh (+dark) found 0 console errors and 1 real issue: in dark mode status alert tints (info/success/warning) stayed LIGHT while `text-foreground` flips light → unreadable. Fixed: per-theme `.dark` blocks in themes.css override status `-background` → dark low-chroma tints (`:root.dark:not([data-theme])` + `[data-theme=…].dark` + `.dark [data-theme=…]` cover co-located and ancestor placements, out-specify base blocks); main/-foreground untouched so solid Badges unaffected. alert.tsx warning icon: added `dark:[&>svg]:text-warning` (dark `-foreground` icon invisible on dark tint → flips to bright amber main). Review subagent fixes integrated: `:root.dark:not([data-theme])` (kill source-order tie with uzh), darkened tints to widen icon contrast. Verified: tsc/build/format clean; agent-browser dark alert both themes readable (icons + text); neutral-dark Badge un-regressed.
- [x] S8 Spinner + Kbd + Empty primitives. Added latest shadcn new-york-v4 source as raw primitives (`ui/spinner.tsx` Loader2 + role=status/aria-label; `ui/kbd.tsx` Kbd + KbdGroup w/ tooltip-aware variant; `ui/empty.tsx` Empty/Header/Media[cva default|icon]/Title/Description/Content) + thin DF wrappers (`Spinner.tsx`, `Kbd.tsx`, `Empty.tsx`) + Ladle stories (README + AI_DOCUMENTATION) + `index.ts` exports. Review subagent fixes: KbdGroup type `'div'`→`'kbd'` (renders kbd); EmptyDescription type `'p'`→`'div'` (renders div); Kbd story doc "div"→"kbd". Rejected reviewer "add border" — shadcn Empty is intentionally borderless (dashed-border opt-in). Verified tsc/ladle-build/prettier clean. Commit `ef115e8`.
- [x] S9 Item + ButtonGroup primitives. Raw `ui/item.tsx` (Item[cva variant default|outline|muted, size default|sm, asChild]/Media[default|icon|image]/Content/Title/Description/Actions/Header/Footer/Group/Separator) + `ui/button-group.tsx` (ButtonGroup[orientation]/Separator/Text + `buttonGroupVariants`) + wrappers + stories + exports. Adapted upstream: `import {Slot} from 'radix-ui'`→`@radix-ui/react-slot`, `Slot.Root`→`Slot`; `@/registry/.../separator`→`./separator`; added `import * as React` to button-group (upstream relied on global React). Review DONE_WITH_CONCERNS, both non-issues (itemVariants-not-exported = faithful to upstream; ItemHeader/Footer documented-but-not-story-imported = stories import only what they render). Verified tsc/ladle-build/prettier clean. Commit `7ff8264`.
- [x] S10 InputGroup + Field primitives. Raw `ui/input-group.tsx` (InputGroup/Addon[cva align inline|block start|end + onClick focus]/Button[cva size xs|sm|icon-xs|icon-sm, targets raw variant `./button`]/Text/Input/Textarea) + `ui/field.tsx` (Field[cva orientation vertical|horizontal|responsive]/Set/Legend/Group/Content/Label/Title/Description/Separator/Error[de-dup via `React.useMemo`+`Map`]) reusing existing `./button`/`./input`/`./textarea`/`./label`/`./separator` + wrappers + stories + exports. Adapted: `useMemo`→`React.useMemo` (+ `import * as React`). Field doc clarifies it is presentation-only; Formik* components stay for managed state. Review flagged 🔴 InputGroupButton size — DISPROVEN: stripping `size`/applying via className is verbatim upstream and `cn`/tailwind-merge resolves padding (confirmed visually in S11, Invite button compact + correct). `== 1` kept (verbatim upstream raw primitive). Verified tsc/ladle-build/prettier clean. Commit `fb2f200`.
- [x] S11 agent-browser visual verification. Ladle `dev` + agent-browser screenshots of all 7 new components, neutral/light + a uzh+dark toggle on Item group. Confirmed: InputGroup+button sizing correct (reviewer 🔴 disproven); Item group icon-media/title/desc/chevron/separator/asChild-links; Field error → red label (invalid propagation) + destructive input border + alert error text; Empty tinted badge + borderless dashed container + primary CTA; ButtonGroup joined Sort/Newest/Oldest segments. uzh+dark: item media boxes flip dark, icons/text flip light — new components inherit the `--theme-*` + `.dark` cascade (canvas stays Ladle-white, expected via `contents`).
- [x] Final security review (S8-S10 commits). Subagent: 0 Critical/Important/Minor. No dangerouslySetInnerHTML/eval/innerHTML; InputGroupAddon onClick = read+focus only (no write/clobber); FieldError renders messages as auto-escaped React text + `Map` keying is prototype-pollution-safe; no secrets/PII; InputGroupButton defaults `type="button"`.
- All slices + the 7-component addition + finish complete. Awaiting: user decision on merge strategy (squash to `main` vs keep `v5` parallel), per-app `data-theme="uzh"` adoption, Google Fonts keep-vs-self-host.
...
- Dark-mode status surfaces: DONE (S7). Status alert tints now dark-adapt per theme. Remaining dark gaps (out of scope, low priority): Error alert keeps shadcn-default dark destructive (description slightly faint — pre-existing shadcn behavior, not the new token layer); nested `<html class=dark><body data-theme=uzh>` placement relies on the descendant selector (covered) but a theme nested under a separate dark element with an intervening re-declared theme is the same general nesting caveat the whole `--theme-*` system has.
