...
- **D6 done** `bd602af` (separator/skeleton/progress) + `1e09789` (tabs). Skeleton bg-muted/r4/shimmer-1.6s (+keyframe); Separator vertical min-h-16; ui/progress track grey/r-sm; Tabs underline strip (gap28, py3/px0, active 600, grid-forcing dropped — VISUALLY BREAKING). Verified Ladle. tsc clean.
  - **Deferred (no reference component / no repro):** Tag, Kbd, Carousel, Countdown, CycleProgress (no `.jsx` reference); Workflow `hover:text-black` (not found in code); Chart aspect-video (no ref + theming already fixed `873419a`, removal risks default sizing). These are NOT pixel-matchable without a spec → out of scope unless design-owner provides one.
...
- **Next: D7** Avatar + Badge + Calendar (reference-backed only).
