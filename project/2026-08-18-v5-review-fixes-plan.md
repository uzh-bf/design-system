...
- S5 Finish Gate: fresh full verification green on 0d1d237a7 (check/lint/format, theme-contract 672, smoke 480, a11y 795, contracts 32, visual 23/23, protocol script, build). Final review `_local/reviews/2026-08-18-package-final-review.md` (DONE_WITH_CONCERNS→integrated): 3 low doc findings, all fixed in 2d6e882a1 (export-table cell, first-paint/match-the-shell-attribute guidance in three places, Collapsible icon-only customTrigger migration note); reviewer independently pixel-verified the 9 regenerated baselines and confirmed all six fixes non-vacuous. Reviewer's least-confident area: the upload/download artifact layout round-trip — confirmed by the PR's green a11y-protocol run.
- PR #207 opened (draft, into v5). First CI run surfaced a latent defect the new blocking VRT gate was built to catch: the visual step had silently exited 1 after 23/23 passing tests on every historical run (masked by continue-on-error). Root causes fixed on-branch: ce792f308 (verdict = docker's PIPESTATUS entry, not the pipeline status; producer failures fail closed with per-stage codes) and 067449363 (non-login container shell — runner uid 1001 collides with the Playwright image's pwuser, whose skeleton .bash_logout runs clear_console, failing without a TTY and overriding exit 0).
- CI gate proof complete: green run 32129107545 (first-ever CI-green VRT); deliberate red 1fbbb428a → run 32130185950 (VRT failed on the intended 51k-pixel canary mismatch, Build+Publish skipped); revert 50789982b → final green run 32130623313 (also confirms the a11y-protocol artifact round-trip, the final reviewer's least-confident area).
- Next: merge #207 into v5 (squash, campaign convention), then Package B (fold hand-written Unreleased CHANGELOG into the generated alpha.5 entry; STOP before tag push).
