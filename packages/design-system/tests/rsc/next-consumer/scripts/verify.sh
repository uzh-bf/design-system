...
for (const declaration of ['index.d.ts', 'primitives.d.ts']) {
  const code = fs.readFileSync(path.join(distDir, declaration), 'utf8')
  if (rhfImport.test(code)) {
    throw new Error(`${declaration} reaches react-hook-form`)
  }
}
console.log(`RSC graphs clean: root ${rootModules} modules, primitives ${primitiveModules} modules`)
NODE

node --input-type=module - "$package_dir/src" "$dist_dir" <<'NODE'
import fs from 'node:fs'
import path from 'node:path'
...
function walk(directory) {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (/\.(ts|tsx)$/.test(entry.name) && !entry.name.endsWith('.d.ts')) sourceFiles.push(file)
  }
}
...
for (const source of sourceFiles) {
  if (!directive.test(fs.readFileSync(source, 'utf8'))) continue
  const relative = path.relative(sourceRoot, source).replace(/\.(ts|tsx)$/, '.js')
  const emitted = path.join(distRoot, relative)
  if (!fs.existsSync(emitted)) continue
  checked += 1
  if (!directive.test(fs.readFileSync(emitted, 'utf8'))) missing.push(relative)
}

if (missing.length > 0) throw new Error(`emitted client directives missing: ${missing.join(', ')}`)
console.log(`preserved ${checked} emitted use client directives`)
NODE
