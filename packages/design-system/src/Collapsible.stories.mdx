...
The `Collapsible` component provides an advanced collapsible interface with enhanced action buttons, conditional content, and extensive customization options. Unlike the raw shadcn Collapsible (from `@uzh-bf/design-system/primitives`), this component offers action buttons, state-dependent content display, and complex trigger customization.
...
**Note**: This is distinct from the raw shadcn Collapsible (available from `@uzh-bf/design-system/primitives`), offering advanced action handling and state-dependent content display capabilities.
...
- ariaLabel?: string - Accessible name for the trigger button (translatable; overrides the English default-trigger fallback)
...
- Collapsible (from @uzh-bf/design-system/primitives): raw collapsible without actions
...
<Collapsible
  ...
  className={{
    ...
    trigger: 'text-destructive-text',
    content: 'text-primary-100',
    staticContent: 'text-primary-100 font-semibold',
    closedContent: 'text-warning-foreground',
  }}
>
  Dynamic content
</Collapsible>
