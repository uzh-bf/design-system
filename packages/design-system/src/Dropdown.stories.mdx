...
The `Dropdown` component provides a sophisticated dropdown menu interface with support for complex item types, custom triggers, and advanced interaction patterns. Unlike the raw shadcn DropdownMenu (from `@uzh-bf/design-system/primitives`), this component offers radio groups, checkboxes, submenus, keyboard shortcuts, and extensive customization options for complex menu scenarios.
...
**Note**: This is distinct from the raw shadcn DropdownMenu (available from `@uzh-bf/design-system/primitives`), offering advanced menu types and complex interaction patterns beyond basic dropdown functionality.
...
- DropdownMenu (from @uzh-bf/design-system/primitives): raw dropdown functionality
