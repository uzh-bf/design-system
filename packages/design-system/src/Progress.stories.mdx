...
**Note**: This is distinct from the raw shadcn Progress (available from `@uzh-bf/design-system/primitives`), which provides basic progress indication without advanced formatting and stacking features.
...
- Progress (from @uzh-bf/design-system/primitives): raw progress indication
  ...
  return (
    <div className="flex flex-col gap-4">
      <Progress
        ...
        max={100}
        formatter={(val) => val}
        className={{ indicator: ['bg-primary-100', 'bg-primary-60'] }}
      />
      <Progress
        value={[70, 20]}
        max={100}
