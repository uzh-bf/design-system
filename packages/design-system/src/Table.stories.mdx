...
**Note**: This is distinct from the raw shadcn Table (available from `@uzh-bf/design-system/primitives`), which provides basic table structure without advanced data manipulation features.
...
- Table (from @uzh-bf/design-system/primitives): raw table structure without advanced features
      ...
      <Table
        columns={[
          ...
          },
          {
            ...
            formatter: ({ row }) => (
              <div className="bg-destructive-background text-destructive-text">
                {row['answer']}
              </div>
            ),
          },
