...
- @param tabs - An array of tab objects, each containing an optional id, label, value, disabled property, and optional data attributes. The tab id is only used as the React list key; the trigger and panel ids in the DOM are always derived from the tabs id and the tab value, so the two ends of the ARIA relation always match.
...
- id?: string - Optional React list key for the tab; never rendered as a DOM id
...
- Flexible: Each tab has an optional id (list key), a value, a label, and optional data attributes
...
- Trigger and panel ids derived from the tabs id and tab value, so aria-controls and aria-labelledby always resolve
...
- data attributes for unique element identification (tab ids never reach the DOM)
